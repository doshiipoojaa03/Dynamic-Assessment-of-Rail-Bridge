 ...
 * @linkcode ./public/pyscript_engineers_web.py
...
export async function runAnalysisWithInputsUI(params: {
  initial: number;
  final: number;
  step: number;
  time_step: number;
  bridge_type: string;
  damping_input?: string | null;
  track_group: string;
  girder_group: string;
  file: File;
  globalkey : string;
}) {
  return checkPyScriptReady(async () => {
    const {
      initial,
      final,
      step,
    ...
    // ✅ Use PyScript's built-in file writer
    // const fileBuffer = await file.arrayBuffer();
    // const fileName = file.name;
    // await pyscript.write_file(fileName, new Uint8Array(fileBuffer)); // ✅ PyScript 2024+ compatible
    const arrayBuffer = await file.arrayBuffer();
    const workbook = XLSX.read(arrayBuffer, { type: "array" });
    const sheet = workbook.Sheets[workbook.SheetNames[0]];
    const fileData2D = XLSX.utils.sheet_to_json(sheet, { header: 1 });
    const jsonData = JSON.stringify(fileData2D);
    
    // ✅ Call Python function from pyscript_main.py
    const pyFunc = pyscript.interpreter.globals.get("run_analysis_with_inputs");

    if (typeof pyFunc !== "function") {
      throw new Error("Python function 'run_analysis_with_inputs' is not available.");
    }
    ...
    try {
      ...
      throw new Error(`Invalid JSON result from Python: ${result}`);
    }
  });
}
