...
return (
    <Panel variant="box" style={{maxWidth: '1450px',width: '1400px',display: 'flex',flexDirection: 'column',alignItems:'center',alignContent:'flex-start',flexWrap:'wrap',gap: '20px',padding:'0', }}>
        <Panel width="fit-content" height="auto" marginTop={3} style={{display: 'flex',flexDirection: 'column',margin: '0 auto',padding: '20px',alignItems:'center',justifyContent:'center'}}>
              {/* Panel Header */}
              <Panel width="100%" height="50px" variant="box">
                <Typography variant="h1" color="primary" center size="large">Dynamic Analysis of Rail Bridge</Typography>
              </Panel>

              {/* Body Content */}
              <div style={{display: 'flex',flexWrap: 'wrap', flexDirection: 'row',gap: '20px',padding: '0px',}}>
                {/* Left Column - Form */}
                <Panel width="480px" height="445px">
                  ...
                  <Panel width="460px" height="70px" variant="strock" marginTop={1}>
                    <Grid container direction="row">
                      <Grid item xs={4}>
                        ...
                        <div style={{ marginTop: '10px' }}>
                          <DropList itemList={bridgeTypes} width="190px" value={bridgeType} onChange={(e) => handleBridgeTypeChange(e.target.value)}/>
                        </div>
                      </Grid>
                      <Grid item xs={4} marginLeft={10}>
                        ...
                        <div style={{ marginTop: '10px' }}>
                          {bridgeType === "User defined" ? (
                            <TextField width="190px" value={damping} onChange={(e) => setDamping(e.target.value)} placeholder="Enter value < 1"/>) : ( <TextField width="190px" value={damping} onChange={() => {}} disabled placeholder="Disabled"/>)}
                        </div>
                      </Grid>
                    </Grid>
                  </Panel>
                  ...
                  <Panel width="460px" height="150px" variant="strock" marginTop={1}>
                    ...
                    <div style={{ marginTop: '15px' }}>
                      ...
                      <Button color="normal" width="20px" onClick={() => fileInputRef.current?.click()}>Browse</Button>
                      <input type="file" style={{ display: 'none' }} accept=".csv,.xlsx" ref={fileInputRef} onChange={handleFileSelect}/>
                    </div>
                    <Grid container direction="row">
                      <Grid item xs={4}>
                        <Typography marginTop={2} variant="body2">Rail Track Nodes</Typography>
                        <div style={{ marginTop: '16px' }}>
                          <DropList itemList={availableGroups}  width="190px" value={railTrackNode} onChange={(e) => setRailTrackNode(e.target.value)} /> 
                        </div>
                      </Grid>
                      <Grid item xs={4} marginLeft={10}>
                        <Typography marginTop={1} variant="body2">Acceleration Output Nodes</Typography>
                        <div style={{ marginTop: '10px' }}>
                          <DropList itemList={availableGroups} width="190px" value={accelerationNode} onChange={(e) => setAccelerationNode(e.target.value)} />
                        </div>
                      </Grid>
                    </Grid>
                  </Panel>
                  ...
                  <Panel flexItem justifyContent="center" width="450px" height="80px" variant="box" marginTop={1} >
                    <Button color="normal" width="auto"  onClick={handleRunAnalysis}>Run Analysis</Button>
                    <div style={{ marginLeft:"10px" }}>
                      <Button color="normal" width="auto" onClick={handleReset}>Refresh</Button>
                    </div>
                  </Panel>
                </Panel>
...
const App = () =>(
<SnackbarProvider maxSnack={5} autoHideDuration={null} hideIconVariant anchorOrigin={{ vertical: 'top', horizontal: 'center' }}
  action={(snackbarId) => (<button onClick={() => closeSnackbar(snackbarId)} style={{background: 'none',border: 'none',color: '#fff',fontSize: '16px',fontWeight: 'bold',marginLeft: '12px',cursor: 'pointer',}}> × </button>)}>
  <WrappedApp />
</SnackbarProvider>
);
export default App;